  ...
  1: [function (require, module, exports) {
    ...
    async function findCallExpression(func, WebCryptoAPIScripts) {
      ...
      for (let j = 0; j < calls.length; j++) {
        ...
        do {
          if (node.property) {
            if (node.property.type === Expr[i].type) {
              if (node.property.type === "Identifier" && node.property.name === Expr[i].name) {
                node = node.object;
                i++;
              }
              else if (node.property.type === "Literal" && node.property.value === Expr[i].value) {
                node = node.object;
                i++;
              }
              else {
    ...
    function makeArray(functionName) {
      let result = [];
      let stop = false
      do {
        if (functionName.type === "Identifier") {
          ...
          result.push(functionName.property);
          functionName = functionName.object;
        }
      }
      while (stop === false)
      return result;
    ...
    window.Regel5 = async function (WebCryptoAPIScripts) {
      let j = 0;
