...
async function func1(signatur) {
    let cipher = await window.crypto.subtle.encrypt({ name: "AES-GCM", iv: window.crypto.getRandomValues(new Uint8Array(16)), }, await AESKeyR3T5, signatur);
    console.log(cipher, signatur);
    return signatur;
}

async function func2(signatur) {
    let cipher = await window.crypto.subtle.encrypt({ name: "AES-GCM", iv: window.crypto.getRandomValues(new Uint8Array(16)), }, await AESKeyR3T5, klarTextR3T5);
    console.log(cipher, signatur);
    return cipher;
}
...
SignKeyR3T5.then(result => window.crypto.subtle.sign({ name: "RSASSA-PKCS1-v1_5" }, result.privateKey, klarTextR3T5)).then(
    async function(result) {window.crypto.subtle.encrypt({ name: "AES-GCM", iv: window.crypto.getRandomValues(new Uint8Array(16)), }, await AESKeyR3T5, result)}).then(console.log);
SignKeyR3T5.then(result => window.crypto.subtle.sign({ name: "RSASSA-PKCS1-v1_5" }, result.privateKey, klarTextR3T5)).then(
    async function(result) {return result}).then(console.log);
Sign1R3T5().then(async function(result) {window.crypto.subtle.encrypt({ name: "AES-GCM", iv: window.crypto.getRandomValues(new Uint8Array(16)), }, await AESKeyR3T5, result)}).then(console.log);
Sign1R3T5().then(async function(result) {return result}).then(console.log);
Sign2R2T1.O4[2].then(async function(result) {window.crypto.subtle.encrypt({ name: "AES-GCM", iv: window.crypto.getRandomValues(new Uint8Array(16)), }, await AESKeyR3T5, result)}).then(console.log);
Sign2R2T1.O4[2].then(async function(result) {return [result, await AESKeyR3T5]}).then(result => window.crypto.subtle.encrypt({ name: "AES-GCM", iv: window.crypto.getRandomValues(new Uint8Array(16)), }, result[1], result[0])).then(console.log);
